...
## Tese editorial obrigatoria

Antes de escrever, formular a tese do artigo em 5 blocos:
...
## Dores prioritarias do publico varejista

Use essas dores como ponto de partida e adapte ao segmento:

- Cliente compra uma vez e some.
- Loja depende de anuncio, indicacao ou movimento da rua para vender de novo.
- Equipe nao pede cadastro no caixa porque o processo e chato ou sem incentivo.
- Cliente antigo fica esquecido no WhatsApp ou na planilha.
- A loja da desconto para todo mundo e perde margem.
- Cashback ou pontos sao usados sem calculo e viram custo sem controle.
- Promocao atrai cliente oportunista, mas nao cria recorrencia.
- O dono nao sabe quem voltou, quem sumiu e quem merece campanha.
- Datas comemorativas sao improvisadas em cima da hora.
- Recompensa e dificil de explicar, entao a equipe nao oferece.
- Loja pequena acha CRM coisa de empresa grande.
- O cliente ate gosta da loja, mas nao recebe motivo claro para voltar.
...
- Cadastro de clientes e formacao de base propria.
- QR/link no balcao para facilitar adesao.
- Campanhas e comunicacao para reativar clientes.
...
1. Abertura: nomear a dor com linguagem de lojista.
2. Diagnostico: explicar por que a dor acontece na rotina real.
...
- Dor principal do varejo.
- Segmento ou contexto de loja quando fizer sentido.
...
- Como fazer o cliente que comprou uma vez voltar sem depender so de anuncio.
- Como pedir cadastro no caixa sem travar a fila.
- Como usar cashback sem destruir a margem da loja.
- Como recuperar clientes inativos com uma campanha simples.
- Como criar uma recompensa que a equipe consegue explicar em 15 segundos.
- Como transformar datas comemorativas em recompra, nao apenas desconto.
- Como uma loja pequena pode usar CRM sem complicar a operacao.
- Como medir se seu programa de fidelidade esta gerando recompra.
- Como trocar desconto generico por beneficio com regra e validade.
- Como montar uma rotina semanal de fidelizacao no comercio local.

## Redacao

- Escrever em portugues brasileiro claro, direto e util.
- Falar com lojistas locais, evitando jargao SaaS pesado.
- Usar exemplos praticos sem inventar marcas reais.
- Incluir cenas de loja: caixa, balcao, WhatsApp, equipe, campanha, cliente sumido, fim de mes, margem e ticket.
- Nao fazer conteudo generico demais: incluir passos, criterios, formulas simples, checklists ou cenarios.
...
Cada artigo deve conter pelo menos dois destes blocos:

- Checklist de implantacao.
- Exemplo de regra de pontos/cashback sem percentual fixo universal.
- Roteiro para equipe usar no caixa.
- Mini-plano de 7 dias.
- Indicadores para acompanhar.
- Erros comuns a evitar.
- Cenario antes/depois sem prometer resultado garantido.
- Perguntas que o dono deve responder antes de criar campanha.

## Capas dos artigos

Usar `image_generate` com `openai/gpt-image-2` quando disponivel para gerar capa editorial 16:9.
...
- Validar visualmente se a capa nao tem texto, numero, logo falsa, marca d'agua, rosto identificavel ou arte confusa.
- Nomear arquivo com slug descritivo em `.webp`.
- Subir para o diretorio publico do blog.
- Confirmar que cada artigo usa uma imagem unica; comparar tamanho e SHA-256 quando houver multiplas capas.

## Implementacao no site

O codigo pode mudar, entao sempre inspecionar antes. Em geral:

1. Encontrar onde os artigos sao definidos/renderizados.
2. Adicionar o artigo seguindo o padrao existente, sem refatoracao fora do escopo.
3. Adicionar imagem publica em `client/public/blog` ou no diretorio equivalente existente.
4. Atualizar card da listagem `/blog`.
5. Atualizar pagina do artigo.
6. Configurar metadados:
   - `title`
   - `description`
   - canonical
   - `robots: index,follow`
   ...
   - `og:image`
   - `twitter:image`
   - schema `BlogPosting` quando o padrao existir
7. Atualizar `sitemap.xml` se ele for estatico ou gerado por lista local.
...
- `title`, `description`, canonical, `og:image` e schema aparecem no HTML server-side quando aplicavel.
- `sitemap.xml` lista o artigo.
- O artigo nao esta com `noindex`.
- Se publicou mais de uma capa, os hashes publicos sao diferentes.
...
Quando a execucao vier de uma tarefa diaria:

- Manha: publicar um artigo mais educativo/topo-meio de funil, sempre ancorado em uma dor real e em um insight pratico.
